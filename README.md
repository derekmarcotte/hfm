...
In practice, the overhead of spawning a new process per test may limit the
frequency that can be achieved by the tests, and their results.  Anecdotally,
2ms intervals have been seen to be achievable, although will depend on your
hardare, operating system, and the configured kernel tick frequency.
...
Scheduling is managed using Go's time.Timer, and time.Ticker, which generally
only guarantees it "pauses the current goroutine for at least the duration d".
I've witnessed up to 75ms tick epsilon.  Therefore tests will run after "at
least" the interval you specify.  Scheduling in this way ensures that we are
not creating a backlog (or flood) of tests, should a test execute for longer
